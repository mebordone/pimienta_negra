...
### 5.10. Dominio del nodo configurable (rebrand `*.local`) — *entregado*

- **Implementación**:
  - Se agregó `NODE_DOMAIN` en `.env`/`.env.example` como origen de verdad (default `pimienta.local`).
  - `accounts.<dominio>` y `conference.<dominio>` se derivan automáticamente para XMPP.
  - nginx, Prosody y Converse ahora se renderizan desde templates en primer arranque (`ops/render-domain-config.sh`).
  - Certificados y registro de admin se alinean al dominio derivado (`init-chat`, `prosody-entrypoint`).
  - mDNS publica `NODE_DOMAIN` + subdominios XMPP.
  - scripts de verificación/tests/CI aceptan dominio default y alternativo.
- **Decisión de operación**:
  - El dominio se define antes del primer bootstrap.
  - No se soporta cambio en caliente con datos existentes; para cambiar dominio se requiere instalación limpia.
